...
if __name__ == "__main__":
    ...
    for device_name, device_id in device_id_list.items():
        ...
        remove_unwanted_keys(device.keys_ts, device_name=device_name)
    ...
    for device_name in device_id_list:
        ...
        _interpolated = postprocess.Interpolation(
                ...
                ref_data = data[device_name])

        for key in devices[device_name].keys_ts:
            ...
            if key == "rain":
                ...
                rain_sum = devices[device_name].get_timeseries(
                ...
                _interpolated.df = _interpolated.df.join(rain_df.rename_axis("date_time"), how="inner")
            else:
...
def camera_id_to_strptime_format(id_):
    return f"%Y-%m-%d_%H%M_phosphatehill-camera-{id_}"

def get_cam_image_folder_from_id(id_):
    folder = (
        "cam1_1down",
        "cam2_2down_3down",
        "cam3_3up_4up",
...
for cam_id in range(1, 7):
    ...
    images[cam_id].map_to_filename_datetime(
...
## Handle axis
for ax_name, axd in ax.items():
    # axd.xaxis.set_visible(False)
    # axd.xaxis.label.set_visible(False)
    axd.tick_params(axis='x', labelbottom=False)
    val_match = re.match(r"^(map|cam).*$", ax_name)
    ...
    if ax_name == "rain":
        ...
        val_match = re.match(r"^(mos)_(\w+)_(\d?)$", ax_name)
        if val_match is not None:
            if val_match.group(1) == "mos":
                axd.set_ylim(bottom=20, top=70)
                ...
                if val_match.group(3) == "4":
                    ...
                    axd.xaxis.set_major_formatter(date_form)
                    axd.tick_params(axis='x', labelrotation=15, labelbottom=True)
...
for image_index, _datetime in enumerate(map_values):
    for axes in ax:
        ...
        if val_match:
            if image_index == 0:
                avxlines[axes] = ax[axes].axvline(_datetime, color='r')
            else:
                avxlines[axes].set_xdata([_datetime, _datetime])
    for cam_id in range(1, 7):
        img = plt.imread(images[cam_id].mapped.filepaths[image_index])
        if image_index == 0:
