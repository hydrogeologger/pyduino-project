...
CSV_FILE_NAME = 'phosphate_hill-1.csv'
...
field_name = ["battery",
            ...
            'suct1','suct2','suct3','suct4','suct5','suct6','suct7','suct8','suct9', 'suct10', 'suct11', 'suct12',
            'oxy1','oxy2','oxy3','oxy4',
...
if (PUBLISH_TO_THINGSBOARD):
    ...
    try:
        ...
        client = mqtt.Client(client_id="phosphatehill_1")
        client.username_pw_set(credential['access_token_phosphatehill_1'])
...
try:
    ...
    if SAVE_TO_CSV:
        ...
        if not os.path.isfile(CSV_FILE_NAME):
            ...
            csv_fid.write("Timestamp, Temp_Internal (C),Humidity_Internal (%)," \
                        ...
                        "SUCT9 (C),SUCT10 (C),SUCT11 (C),SUCT12 (C)," \
    ...
    try:
        ...
        command = "9548,1,type,sht31,points,3,dummies,1,debug,0"
        ard.write(command.encode())
        ard.flushInput()
        msg = ard.readline().decode() # <temperature>,humidity,
        current_read = msg.split(',')[0:-1]
    ...
    try:
        ...
        wind_dir_device = WindDirection(name=sensor_name, offset=270)
        GPIO.setmode(GPIO.BCM)
        rpi_pin = 6
        GPIO.setup(rpi_pin, GPIO.OUT)
        GPIO.output(rpi_pin, GPIO.HIGH)
        command = "analog,9,power,9,points,3,dummies,1,interval_mm,100,debug,1"
        ard.write(command.encode())
        ard.flushInput()
    ...
    try:
        sensor_name = "suct9"
        command = "fred,___,dgin,3,snpw,8,htpw,46,itv,6000,otno,5,debug,0"
        ard.write(command.encode())
        ard.flushInput()
        msg = ard.readline().decode() #<addr>,<base temp>,<heat temp_0>,<heat temp_...>,<cool temp_0>,<cool temp_...>
        current_read=msg.split(',')[0:-1]
        ...
        data_collected['suct9'] = temp_diff
        if SCREEN_DISPLAY:
            print(sensor_name.upper() + ": " + msg.rstrip())
        if SAVE_TO_CSV:
            csv_fid.write(DELIMITER + str(temp_diff))
    ...
    try:
        sensor_name = "suct10"
        command = "fred,___,dgin,4,snpw,8,htpw,47,itv,6000,otno,5,debug,0"
        ard.write(command.encode())
        ard.flushInput()
        msg = ard.readline().decode() #<addr>,<base temp>,<heat temp_0>,<heat temp_...>,<cool temp_0>,<cool temp_...>
        current_read=msg.split(',')[0:-1]
        ...
        data_collected['suct10'] = temp_diff
        if SCREEN_DISPLAY:
            print(sensor_name.upper() + ": " + msg.rstrip())
        if SAVE_TO_CSV:
            csv_fid.write(DELIMITER + str(temp_diff))
    ...
    try:
        sensor_name = "suct11"
        command = "fred,___,dgin,5,snpw,8,htpw,48,itv,6000,otno,5,debug,0"
        ard.write(command.encode())
        ard.flushInput()
        msg = ard.readline().decode() #<addr>,<base temp>,<heat temp_0>,<heat temp_...>,<cool temp_0>,<cool temp_...>
        current_read=msg.split(',')[0:-1]
        ...
        data_collected['suct11'] = temp_diff
        if SCREEN_DISPLAY:
            print(sensor_name.upper() + ": " + msg.rstrip())
        if SAVE_TO_CSV:
            csv_fid.write(DELIMITER + str(temp_diff))
    ...
    try:
        sensor_name = "suct12"
        command = "fred,___,dgin,64,snpw,8,htpw,49,itv,6000,otno,5,debug,0"
        ard.write(command.encode())
        ard.flushInput()
        msg = ard.readline().decode() #<addr>,<base temp>,<heat temp_0>,<heat temp_...>,<cool temp_0>,<cool temp_...>
        current_read=msg.split(',')[0:-1]
        ...
        data_collected['suct12'] = temp_diff
        if SCREEN_DISPLAY:
            print(sensor_name.upper() + ": " + msg.rstrip())
        if SAVE_TO_CSV:
            csv_fid.write(DELIMITER + str(temp_diff))
    ...
    try:
        ...
        GPIO.setup(rpi_pin, GPIO.OUT)
        GPIO.output(rpi_pin, GPIO.HIGH)
        command = "luminox,A,power,-1,serial,1,debug,0"
        ard.write(command.encode())
        ard.flushInput()
    ...
    try:
        ...
        GPIO.setup(rpi_pin, GPIO.OUT)
        GPIO.output(rpi_pin, GPIO.HIGH)
        command = "luminox,A,power,-1,serial,3,debug,0"
        ard.write(command.encode())
        ard.flushInput()
        msg = ard.readline().decode() # “O xxxx.x T yxx.x P xxxx % xxx.xx e xxxx\r\n”
        current_read=msg.split(',')[-1].split(' ')
    ...
    try:
        ...
        GPIO.setup(rpi_pin, GPIO.OUT)
        GPIO.output(rpi_pin, GPIO.HIGH)
        command = "luminox,A,power,-1,serial,2,debug,0"
        ard.write(command.encode())
        ard.flushInput()
        msg = ard.readline().decode() # “O xxxx.x T yxx.x P xxxx % xxx.xx e xxxx\r\n”
        current_read=msg.split(',')[-1].split(' ')
